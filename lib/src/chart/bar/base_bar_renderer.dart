...
import 'package:charts_flutter/src/chart/cartesian/cartesian_renderer.dart'
...
import 'package:charts_flutter/src/common/math.dart' show clamp;
...
>
  ...
  List<DatumDetails<D>> _getVerticalDetailsForDomainValue(
    D? domainValue,
    Point<double> chartPoint,
  ) => List<DatumDetails<D>>.from(
    _getSegmentsForDomainValue(
      domainValue,
      where: (bar) => !bar.series.overlaySeries,
...
class _ReversedSeriesIterator<S extends ImmutableSeries<Object?>>
  ...
  _ReversedSeriesIterator(List<S> list) : _list = list {
    ...
    categoryAndSeriesIndexMap.forEach(
      (_, indices) => _visitIndex.addAll(indices.reversed),
    );
  }
  final List<S> _list;

...
import 'package:charts_flutter/src/chart/cartesian/axis/draw_strategy/none_draw_strategy.dart'
...
abstract class CartesianChart<D> extends BaseChart<D> {
  ...
  void init(ChartContext context, GraphicsFactory graphicsFactory) {
    ...
    _primaryMeasureAxis
      ..context = context
      ..tickDrawStrategy = const GridlineRendererSpec<num>().createDrawStrategy(
        context,
        graphicsFactory,
      );
  ...
  void configurationChanged() {
    ...
    if (_disjointMeasureAxesSpec != _newDisjointMeasureAxesSpec) {
      ...
      _disjointMeasureAxesSpec?.forEach((axisId, axisSpec) {
        ...
        _disjointMeasureAxes[axisId]!.tickDrawStrategy = NoneDrawStrategy<num>(
          graphicsFactory!,
        );
        axisSpec.configure(
  ...
  SeriesRenderer<D> makeDefaultRenderer() =>
      BarRenderer()..rendererId = SeriesRenderer.defaultRendererId;

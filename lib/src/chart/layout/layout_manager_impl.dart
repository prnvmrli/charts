...
import 'package:charts_flutter/src/chart/layout/layout_config.dart'
    show LayoutConfig;
import 'package:charts_flutter/src/chart/layout/layout_manager.dart';
import 'package:charts_flutter/src/chart/layout/layout_margin_strategy.dart';
...
class LayoutManagerImpl implements LayoutManager {
  ...
  List<LayoutView> get positionOrderedViews {
    if (_viewsNeedPositionSort) {
      ...
      _positionOrderedViews.sort(
        (v1, v2) => v1.layoutConfig.positionOrder!.compareTo(
          v2.layoutConfig.positionOrder!,
        ),
      );
  ...
  Rectangle<int> get drawAreaBounds {
    assert(!_drawAreaBoundsOutdated);
  ...
  @override
  bool withinDrawArea(Point<num> point) => _drawAreaBounds.containsPoint(point);

  /// Measure and layout with given [width] and [height].
  @override
  void measure(int width, int height) {
    final topViews = _viewsForPositions(
      LayoutPosition.Top,
      LayoutPosition.FullTop,
    );
    final rightViews = _viewsForPositions(
      LayoutPosition.Right,
      LayoutPosition.FullRight,
    );
    final bottomViews = _viewsForPositions(
      LayoutPosition.Bottom,
      LayoutPosition.FullBottom,
    );
    final leftViews = _viewsForPositions(
      LayoutPosition.Left,
